...
def log_with_details(logger: logging.Logger, level: int, message: str,
...
def validate_config(config: Dict[str, Any]) -> bool:
    ...
    # Support OpenAI (sk-), GLM/Zhipu AI, and other formats - just check not
    # empty and reasonable length
    if len(api_key) < 20:
        return False
    ...
    if model not in valid_models:
        print(f"Warning: Using potentially unsupported model '{
              model}'. Supported models: {', '.join(valid_models)}")
...
def load_config(logger: logging.Logger, config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    for key in ['OPENAI_API_KEY', 'OPENAI_BASE_URL',
                'OPENAI_MODEL', 'LOG_PATH', 'AUTO_COMMIT', 'AUTO_PUSH']:
...
def select_files_interactive(
        staged_files: List[str], unstaged_files: List[str], logger: logging.Logger) -> List[str]:
                    ...
                    try:
                        ...
                        if 1 <= num <= len(unstaged_files):
                            ...
                            print(f"Invalid number: {
                                  num}. Please use numbers 1-{len(unstaged_files)}")
                    except ValueError:
...
def generate_commit_message(
        diff_text: str, config: Dict[str, Any], logger: logging.Logger) -> Optional[str]:
